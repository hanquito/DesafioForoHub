...
import com.challengue.forohub.domain.respuesta.dto.DetalleRespuestaDTO;
import com.challengue.forohub.domain.respuesta.repository.RespuestaRepository;
import com.challengue.forohub.domain.topico.Estado;
...
import com.challengue.forohub.domain.topico.dto.DetallesTopicoDTO;
import com.challengue.forohub.domain.topico.repository.TopicoRepository;
import com.challengue.forohub.domain.topico.validations.create.ValidarTopicoCreado;
import com.challengue.forohub.domain.topico.validations.update.ValidarTopicoActualizado;
import com.challengue.forohub.domain.usuario.Usuario;
import com.challengue.forohub.domain.usuario.repository.UsuarioRepository;
...
@Tag(name="Topico", description = "Esta vinculado a un curso y usuario especificos.")
public class TopicoController {
    ...
    public ResponseEntity<Page<DetallesTopicoDTO>> LeerTodosTopicos(@PageableDefault(size = 5, sort={"nombre"}) Pageable pageable) {
    ...
    public ResponseEntity<Page<DetallesTopicoDTO>> LeerTopicosNoEliminados(@PageableDefault(size = 5, sort={"nombre"}) Pageable pageable) {
        var pagina = topicoRepository.findAllByEstadoIsNot(Estado.DELETED, pageable).map(DetallesTopicoDTO::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping("/{id}")
    @Operation(summary = "Lee un unico tema por su Id")
    ...
    @Operation(summary = "Lee la respuesta del topico marcada como su solucion")
