...
import com.challengue.forohub.domain.usuario.dto.ActualizarUsuarioDTO;
import com.challengue.forohub.domain.usuario.dto.CrearUsuarioDTO;
import com.challengue.forohub.domain.usuario.dto.DetallesUsuarioDTO;
import com.challengue.forohub.domain.usuario.repository.UsuarioRepository;
import com.challengue.forohub.domain.usuario.validations.create.ValidarCrearUsuario;
import com.challengue.forohub.domain.usuario.validations.update.ValidarActualizarUsuario;
...
public class UsuarioController {
    ...
    @GetMapping("/all")
    @Operation(summary = "Enumera todos losusuarios independientesmente de su estado")
    public ResponseEntity<Page<DetallesUsuarioDTO>> leerTodosUsuarios(@PageableDefault(size = 5,sort={"nombre"})Pageable pageable){
        var pagina = repository.findAll(pageable).map(DetallesUsuarioDTO::new);
        return ResponseEntity.ok(pagina);
    }
    @GetMapping
    @Operation(summary = "Lista solo usuarios habilitados")
    public ResponseEntity<Page<DetallesUsuarioDTO>> leerUsuariosActivos(@PageableDefault(size=5, sort={"nombre"})Pageable pageable){
        var pagina = repository.findAllByEnabledTrue(pageable).map(DetallesUsuarioDTO::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping("/username/{username}")
    @Operation(summary = "Lee un unico usuario por su nombre de usuario")
    ...
    @PutMapping("/{username}")
    @Transactional
    @Operation(summary= "Actualiza la contraseña de un usuario, rol, nombre y apelllido, correo")
